...
class AddColumnTab(QWidget):
    def __init__(self, parent=None):
        ...
        # Grilla de preview/resultado (inicialmente oculta)
        self.table = DataTable()
        ...
        # Botones de acción (agrupados como en otros tabs)
        actions = QHBoxLayout()
    ...
    def set_preview(self, df: pd.DataFrame, palette: dict):
        """Muestra el preview del CSV cargado."""
        self._palette = palette
    ...
    def _to_upper(self, line_edit: QLineEdit):
        """Convierte el texto a MAYÚSCULAS manteniendo la posición del cursor."""
        text = line_edit.text()
    ...
    def _show_preview(self):
        """Muestra vista previa de cómo quedará el archivo con la nueva columna."""
        if not self._filepath:
            QMessageBox.warning(self, "Sin archivo", "Cargá un archivo CSV primero.")
            return
        ...
        try:
            # Leer primeras filas (usar mismo límite que PREVIEW_ROWS)
            df = pd.read_csv(self._filepath, encoding=self._enc, delimiter=self._delim, nrows=PREVIEW_ROWS)

            # Determinar posición de la nueva columna
    ...
    def _finish(self, msg, state, out_dir):
        ...
        if out_dir:
            ...
            try:
                ...
                pass  # Si falla, no mostrar grilla
